...
`mocop init` refuses to overwrite an existing configuration. Edit the file directly when the cluster changes, keep automatic discovery disabled, and list only compute-node aliases:
...
This is an inventory excerpt, not a complete replacement configuration. Keep the other fields generated by `mocop init`. `exclude_hosts` is a final deny-list and is especially useful when you intentionally enable `auto_discover`. Restart the service after editing the file.
