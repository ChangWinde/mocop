...
| `mocop brief` | Print the situation brief of the running service (`GET /api/brief`) as one screen of text: fleet status, actionable conditions worst first with correlations and shared filesystems collapsed, openings and recoveries over the window with recurring conditions, idle GPUs by host, per-owner GPU-hours with idle share; exit codes follow `mocop api` | `--hours N` (1–168, default 24), `--json` (the service's document unchanged), `--token-file PATH`, `--timeout SECONDS` |
