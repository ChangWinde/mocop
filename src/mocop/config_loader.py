...
from .config_integrations import (
    connection_topology,
    persistence_config,
    webhook_configs,
    workload_config,
...
def _incident_cycles(data: dict[str, Any]) -> IncidentConfig:
    ...
    seconds = incident_data.get(
        "resource_open_seconds", incident_defaults.resource_open_seconds
    )
    if (
        ...
        raise ConfigError("incidents.resource_open_seconds must be a number")
    if not 0 <= seconds <= 3600:
        raise ConfigError("incidents.resource_open_seconds must be between 0 and 3600")
    ...
    return IncidentConfig(
        ...
        resource_open_seconds=float(seconds),
    )
...
def _parse_config_bytes(content: bytes, config_path: Path) -> MonitorConfig:
    ...
    topology = connection_topology(data["topology"]) if "topology" in data else None
    ...
    persistence = persistence_config(data)
    workloads = workload_config(data)
    webhooks = webhook_configs(data)
