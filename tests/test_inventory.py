...
class InventoryTests(unittest.TestCase):
    ...
    def test_sets_clears_and_avoids_rewriting_shared_host_groups(self) -> None:
        ...
        self.assertEqual(
            dict(load_config(self.config_path).host_groups).get("gpu-01"), "Training"
        )
        ...
        self.assertIsNone(dict(load_config(self.config_path).host_groups).get("gpu-01"))
        raw = json.loads(self.config_path.read_text(encoding="utf-8"))
    ...
    def test_sets_and_clears_bounded_maintenance_windows_atomically(self) -> None:
        ...
        reloaded = load_config(self.config_path)
        self.assertTrue(dict(reloaded.maintenance_windows)["gpu-01"].is_active())
        self.assertEqual(self.config_path.stat().st_mode & 0o777, 0o600)
        ...
        self.assertIsNone(
            dict(load_config(self.config_path).maintenance_windows).get("gpu-01")
        )
