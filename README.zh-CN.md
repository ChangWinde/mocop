...
在居中的 **设置** 工作区中，可以选择五套专门设计的界面皮肤、舒适或紧凑的信息密度、默认服务器关注视图、服务器和 GPU 排序、热力图指标及可见的 GPU 数据列。雾光玻璃与终端矩阵会改变几何、表面质感、层次和字体气质，而不只是换颜色。还可以选择不超过 8 MiB 的 PNG、JPEG、WebP 或 AVIF 背景并调整可见度；Mocop 会校验解码后的尺寸，图片只保存在当前浏览器，绝不会上传到服务端。
